...
const WhatsAppIcon = ({ className }) => (
  <svg 
    ...
    aria-hidden="true" // Otimização Acessibilidade: Ícone decorativo
  >
...
// --- CONFIGURAÇÃO GLOBAL & FONTES (Otimizado com font-display: swap) ---
const GlobalStyles = () => (
  <style>{`
...
const GalleryCarousel = () => {
  ...
  useEffect(() => {
    const handleResize = () => setItemsToShow(window.innerWidth >= 768 ? 3 : 1);
    handleResize();
    window.addEventListener('resize', handleResize);
    return () => window.removeEventListener('resize', handleResize);
  }, []);

  const nextSlide = () => setCurrentIndex((prev) => (prev >= galleryImages.length - itemsToShow ? 0 : prev + 1));
  const prevSlide = () => setCurrentIndex((prev) => (prev === 0 ? galleryImages.length - itemsToShow : prev - 1));
    ...
    <div className="relative w-full max-w-7xl mx-auto px-4 md:px-8 group">
      <AnimatePresence>
        {lightboxImage && (
          ...
          >
            <button className="absolute top-8 right-8 text-white/70 hover:text-white transition" aria-label="Fechar"><X size={40} strokeWidth={1} /></button>
            <img src={lightboxImage} alt="Zoom" className="max-h-[90vh] max-w-full rounded shadow-2xl" onClick={(e) => e.stopPropagation()} loading="lazy" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="overflow-hidden">
        ...
        >
          {galleryImages.map((img, idx) => (
            ...
            >
              <img src={img.src} alt={img.title} className="w-full h-full object-cover transition-transform duration-700 group-hover/item:scale-110" loading="lazy" width="800" height="600" />
      ...
      <button onClick={prevSlide} aria-label="Anterior" className="absolute left-0 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 bg-white shadow-xl rounded-full flex items-center justify-center text-slate-800 hover:text-teal-600 transition-all opacity-80 hover:opacity-100 z-10"><ChevronLeft size={24} /></button>
      <button onClick={nextSlide} aria-label="Próximo" className="absolute right-0 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 bg-white shadow-xl rounded-full flex items-center justify-center text-slate-800 hover:text-teal-600 transition-all opacity-80 hover:opacity-100 z-10"><ChevronRight size={24} /></button>
    </div>
  );
};
    ...
    try {
      if (WEBHOOK_URL) {
        await fetch(WEBHOOK_URL, {
          ...
          body: JSON.stringify({ ...formData, date: new Date().toLocaleString('pt-BR'), origin: 'Landing Page' })
    ...
    <div className="bg-white text-slate-800 font-sans min-h-screen antialiased selection:bg-teal-100 selection:text-teal-900">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm h-24 transition-all duration-300">
        <div className="container mx-auto px-6 h-full flex justify-between items-center relative">
          <div className="hidden lg:flex items-center justify-start gap-6 w-5/12">
          ...
          <button className="lg:hidden text-slate-800 ml-auto" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Menu">
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="lg:hidden bg-white border-t border-slate-100 p-4 flex flex-col gap-3 shadow-xl absolute w-full">
              ...
              <Button variant="whatsapp" onClick={() => handleWhatsApp()} className="w-full mt-2 flex justify-center gap-2"><WhatsAppIcon className="w-5 h-5" /> WhatsApp</Button>
            </motion.div>
          )}
        </AnimatePresence>
      </nav>
      ...
      <div className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 bg-slate-900 overflow-hidden min-h-[calc(100vh-6rem)] flex items-center">
        <div className="absolute inset-0 z-0">
          <img src="https://images.unsplash.com/photo-1527525443983-6e60c75fff46?q=80&w=1920&auto=format&fit=crop" alt="Acolhimento" className="w-full h-full object-cover opacity-20" loading="eager" width="1920" height="1080" />
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <div className="lg:w-1/2 text-center lg:text-left">
              <FadeIn>
                <div className="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-white/5 border border-white/10 text-white text-sm font-semibold mb-8 backdrop-blur-sm"><Clock size={14} /> Atendimento Humanizado 24 horas</div>
                <h1 className="text-4xl lg:text-6xl font-bold text-white mb-8 leading-[1.1] tracking-tight">A recuperação começa com <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-200">um passo.</span></h1>
                <p className="text-xl text-slate-300 mb-10 leading-relaxed font-light max-w-xl mx-auto lg:mx-0">Seja qual for o vício, existe um caminho de volta. Oferecemos tratamento especializado, seguro e acolhedor para reconstruir histórias.</p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                  <Button variant="primary" onClick={scrollToForm} className="min-w-[200px]">Quero Ajuda Agora</Button>
                  <Button variant="outline" onClick={() => handleWhatsApp()} className="min-w-[200px]" aria-label="Falar no WhatsApp"><WhatsAppIcon className="w-5 h-5" /> Falar no WhatsApp</Button>
                </div>
            ...
            <div className="lg:w-1/2 w-full hidden lg:block">
              <FadeIn delay={0.2}>
                <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-white/10 group bg-slate-800">
                  <img src="https://cdn.agsup.com.br/grv/imagem-clinica.jpeg?q=80&w=800&auto=format&fit=crop" alt="Ambiente de Paz" className="w-full h-[450px] object-cover transition-transform duration-1000 group-hover:scale-105" loading="lazy" width="800" height="450" />
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 to-transparent"></div>
                  <div className="absolute bottom-8 left-8 right-8 glass-panel p-6 rounded-xl flex items-start gap-4 z-20">
                    <div className="bg-teal-500 p-3 rounded-full text-white shadow-lg shadow-teal-500/30"><ShieldCheck size={28} strokeWidth={1.5} /></div>
                    <div><p className="font-bold text-slate-900 text-lg">Ambiente Monitorado</p><p className="text-sm text-slate-600">Segurança, paz e tranquilidade fundamentais para a recuperação plena.</p></div>
                  </div>
                </div>
              </FadeIn>
            </div>
      ...
      <Section bg="gray">
        <div className="container mx-auto">
           <div className="text-center mb-12 max-w-3xl mx-auto">
              ...
              <p className="text-slate-500 text-lg font-light leading-relaxed">Nossa equipe multidisciplinar desenvolveu protocolos específicos para cada tipo de dependência. Entendemos que cada substância age de forma diferente no organismo, e por isso, o tratamento precisa ser direcionado.</p>
           </div>
           <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
              {[
                ...
                <motion.div key={i} whileHover={{ y: -5 }} className={`bg-white p-8 rounded-2xl border border-slate-200 flex flex-col items-center justify-center gap-4 cursor-default transition-all duration-300 shadow-sm ${item.hoverBorder} hover:shadow-xl ${item.hoverShadow}`}>
      ...
      <Section className="bg-white text-center max-w-5xl mx-auto">
        <FadeIn>
          <div className="mb-8 flex justify-center"><div className="bg-teal-50 p-6 rounded-full text-teal-600"><Users size={48} strokeWidth={1} /></div></div>
          <h2 className="text-4xl md:text-5xl font-bold text-slate-900 mb-8">Entendemos o que você está passando</h2>
          <p className="text-xl text-slate-500 leading-relaxed mb-12 max-w-3xl mx-auto font-light">Sabemos que pedir ajuda não é fácil. Muitos tentam sozinhos, mas recaem. Aqui, você encontra <strong className="text-teal-700 font-semibold">acolhimento, cuidado e tratamento especializado</strong> — sem julgamentos, focados em recomeços reais.</p>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <FadeIn delay={0.1}>
              <div className="bg-white p-8 rounded-3xl border border-slate-100 h-full shadow-lg hover:shadow-2xl transition-all duration-300 group hover:-translate-y-2">
                <div className="w-16 h-16 bg-teal-50 rounded-2xl flex items-center justify-center mb-8 group-hover:bg-teal-600 transition-colors duration-300"><Stethoscope size={32} className="text-teal-600 group-hover:text-white transition-colors" strokeWidth={1.5} /></div>
                ...
                <ul className="space-y-4">
                  {["Psiquiatra", "Psicóloga", "Enfermeira Padrão", "Terapeutas 24h", "Coordenadores"].map((item, i) => (
                    <li key={i} className="flex items-center gap-3 text-slate-600 font-medium"><div className="w-6 h-6 rounded-full bg-teal-50 flex items-center justify-center text-teal-600"><CheckCircle2 size={14} /></div>{item}</li>
                  ))}
                </ul>
              </div>
            </FadeIn>
            <FadeIn delay={0.2}>
              <div className="bg-white p-8 rounded-3xl border border-slate-100 h-full shadow-lg hover:shadow-2xl transition-all duration-300 group hover:-translate-y-2">
                <div className="w-16 h-16 bg-orange-50 rounded-2xl flex items-center justify-center mb-8 group-hover:bg-orange-500 transition-colors duration-300"><Brain size={32} className="text-orange-500 group-hover:text-white transition-colors" strokeWidth={1.5} /></div>
                <h3 className="text-2xl font-bold text-slate-900 mb-6 border-b border-slate-100 pb-4">Terapias</h3>
                <div className="space-y-5">
                  <div><strong className="text-slate-900 block mb-1 text-lg">P.P.R</strong><span className="text-slate-500 text-sm font-light block">Prevenção a Recaída e ferramentas para a vida.</span></div>
                  <div><strong className="text-slate-900 block mb-1 text-lg">T.R.E</strong><span className="text-slate-500 text-sm font-light block">Terapia Racional Emotiva e sentimentos.</span></div>
                  <div><strong className="text-slate-900 block mb-1 text-lg">Laborterapia</strong><span className="text-slate-500 text-sm font-light block">Disciplina e resgate da autoestima.</span></div>
                </div>
              </div>
            </FadeIn>
            <FadeIn delay={0.3}>
              <div className="bg-white p-8 rounded-3xl border border-slate-100 h-full shadow-lg hover:shadow-2xl transition-all duration-300 group hover:-translate-y-2">
                <div className="w-16 h-16 bg-green-50 rounded-2xl flex items-center justify-center mb-8 group-hover:bg-green-600 transition-colors duration-300"><Utensils size={32} className="text-green-600 group-hover:text-white transition-colors" strokeWidth={1.5} /></div>
                ...
                <FadeIn key={i} delay={i * 0.1}>
                  <div className="bg-slate-900 rounded-2xl overflow-hidden shadow-2xl group h-full flex flex-col">
                     <div className="h-64 overflow-hidden relative">
                        <img src={unit.img} alt={unit.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" width="800" height="450" />
                     ...
                     <div className="p-8 flex flex-col flex-grow">
                        ...
                        <div className="space-y-4 text-slate-300 text-sm mb-8 flex-grow">
                          <div className="flex gap-3"><MapPin className="text-teal-500 shrink-0" size={20} /><span className="text-base">{unit.address}</span></div>
                          <div className="flex gap-3"><Phone className="text-teal-500 shrink-0" size={20} /><span className="text-base">{unit.phone}</span></div>
                          <div className="flex gap-3"><Clock className="text-teal-500 shrink-0" size={20} /><span className="text-base">24 horas - Todos os dias</span></div>
                        </div>
                        <div className="space-y-3">
                          <button onClick={handleCall} className="flex items-center justify-center gap-2 w-full text-white font-medium py-2 hover:text-teal-400 transition" aria-label={`Ligar para ${unit.title}`}><Phone size={16} /> Ligar Agora</button>
                          <Button variant="whatsapp" onClick={() => window.open(unit.whatsappLink, '_blank')} className="w-full py-3 text-sm flex justify-center gap-2 shadow-none"><WhatsAppIcon className="w-5 h-5" /> Falar no WhatsApp</Button>
                        </div>
                     </div>
                  </div>
                </FadeIn>
            ...
            {[
              ...
              <FadeIn key={idx} delay={idx * 0.1}>
                <div className="bg-slate-50 p-10 rounded-t-3xl rounded-br-3xl rounded-bl-none border border-slate-100 text-left hover:shadow-lg transition relative h-full flex flex-col">
                  <div className="flex gap-1 mb-6 text-orange-400">{[1,2,3,4,5].map(star => <Star key={star} size={16} fill="currentColor" />)}</div>
                  <p className="text-slate-600 italic mb-8 flex-grow font-light text-lg">"{depo.text}"</p>
                  <div><p className="font-bold text-slate-900 text-lg">{depo.author}</p><p className="text-xs text-slate-400 uppercase font-bold tracking-widest">{depo.role}</p></div>
                </div>
              </FadeIn>
            ))}
          </div>
            ...
            <div className="lg:w-5/12 p-12 bg-gradient-to-br from-slate-900 to-slate-800 text-white relative overflow-hidden">
               ...
               <div className="relative z-10 h-full flex flex-col justify-between">
                 <div>
                   ...
                   <p className="text-slate-300 text-lg mb-8 font-light leading-relaxed">Nossa equipe de triagem está pronta para te ouvir. O atendimento é sigiloso, seguro e imediato.</p>
                   <div className="space-y-6">
                     <div className="flex items-center gap-4"><div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center text-teal-400"><Phone size={24} /></div><div><p className="text-xs text-slate-400 uppercase font-bold tracking-wider">Plantão 24h</p><p className="text-xl font-bold">{displayPhone}</p></div></div>
                     <div className="flex items-center gap-4"><div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center text-green-400"><MessageCircle size={24} /></div><div><p className="text-xs text-slate-400 uppercase font-bold tracking-wider">WhatsApp</p><p className="text-xl font-bold">{displayPhone}</p></div></div>
                   </div>
                 </div>
                 <div className="mt-12 pt-8 border-t border-white/10"><p className="flex items-center gap-2 text-slate-400 text-sm"><Lock size={16} /> Suas informações estão 100% seguras.</p></div>
               </div>
            </div>
            <div className="lg:w-7/12 p-12 bg-white">
              ...
              <form className="space-y-5" onSubmit={handleSubmit}>
                <div className="grid md:grid-cols-2 gap-5">
                  <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Nome</label><input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full p-4 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none transition" placeholder="Seu nome" required /></div>
                  <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Telefone</label><input type="tel" name="phone" value={formData.phone} onChange={handleChange} className="w-full p-4 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none transition" placeholder="(DDD) 99999-9999" required /></div>
                </div>
                <div className="grid md:grid-cols-2 gap-5">
                   <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Cidade</label><input type="text" name="city" value={formData.city} onChange={handleChange} className="w-full p-4 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none transition" placeholder="Sua cidade" required /></div>
                   <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Como podemos ajudar?</label><select name="subject" value={formData.subject} onChange={handleChange} className="w-full p-4 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-teal-500 focus:border-transparent outline-none transition text-slate-600"><option>Internação para Drogas</option><option>Internação para Álcool</option><option>Vício em Jogos</option><option>Outros</option></select></div>
                </div>
                <Button variant="whatsapp" className="w-full py-5 text-lg shadow-xl shadow-green-500/20 flex justify-center items-center gap-3 disabled:opacity-70" disabled={isSending}>
                   {isSending ? (<>Enviando...</>) : (<><WhatsAppIcon className="w-6 h-6" /> SOLICITAR AJUDA AGORA</>)}
                </Button>
      ...
      <footer className="bg-slate-950 text-slate-400 py-12 text-sm border-t border-slate-800">
        <div className="container mx-auto px-6 flex flex-col items-center gap-6">
            <p className="text-center text-base text-slate-500" style={{ fontFamily: "'Funnel Display', sans-serif" }}>Grupo Restaura Vidas {new Date().getFullYear()} - Feito com <span style={{color: 'red'}}>❤</span> pelo time da <a title="Gostou? Podemos ajudar sua empresa também! Entre em contato" href="https://agenciasupimpa.com.br?utm_source=gruporestauravida&amp;utm_id=clientes" target="_blank" rel="noopener" className="hover:text-teal-500 transition"><b>SUPIMPA</b> - Agência Digital</a></p>
        </div>
      </footer>

      {/* Botão Flutuante WhatsApp */}
      <a href={`https://wa.me/${phoneNumber}`} target="_blank" className="fixed bottom-6 right-6 z-50 hover:-translate-y-1 transition-transform" aria-label="WhatsApp Flutuante">
